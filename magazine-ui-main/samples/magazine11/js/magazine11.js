...
var sampleName = 'magazine11',
	samplePath = 'samples/magazine11/';
...
function loadPage(page) {
	...
	var img = $('<img />');
	img.load(function() {
		var container = $('.magazine11 .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.magazine11 .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.magazine11').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.magazine11').addClass('animated').removeClass('zoom-in');
				}, 0);
			...
			swipeLeft: function() {
				...
				$('.magazine11').turn('next');

			},

			swipeRight: function() {
				
				$('.magazine11').turn('previous');
