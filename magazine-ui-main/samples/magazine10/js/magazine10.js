...
var sampleName = 'magazine10',
	samplePath = 'samples/magazine10/';
...
function loadPage(page) {
	...
	var img = $('<img />');
	img.load(function() {
		var container = $('.magazine10 .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.magazine10 .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.magazine10').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.magazine10').addClass('animated').removeClass('zoom-in');
				}, 0);
			...
			swipeLeft: function() {
				...
				$('.magazine10').turn('next');

			},

			swipeRight: function() {
				
				$('.magazine10').turn('previous');
