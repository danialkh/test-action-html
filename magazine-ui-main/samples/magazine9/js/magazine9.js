...
var sampleName = 'magazine9',
	samplePath = 'samples/magazine9/';
...
function loadPage(page) {
	...
	var img = $('<img />');
	img.load(function() {
		var container = $('.magazine9 .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.magazine9 .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.magazine9').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.magazine9').addClass('animated').removeClass('zoom-in');
				}, 0);
