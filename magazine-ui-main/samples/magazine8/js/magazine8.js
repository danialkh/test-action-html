...
var sampleName = 'magazine8',
	samplePath = 'samples/magazine8/';
...
function loadPage(page) {
	...
	var img = $('<img />');
	img.load(function() {
		var container = $('.magazine8 .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.magazine8 .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.magazine8').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.magazine8').addClass('animated').removeClass('zoom-in');
				}, 0);
